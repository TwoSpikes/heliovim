...
bool findpar(bool *pincl, int dir, int count, int what, bool both)
{
  ...
  if (curr == curbuf->b_ml.ml_line_count && what != '}' && dir == FORWARD) {
    ...
    // motion inclusive.
    if ((cursor->col = ml_get_len(curr)) != 0) {
      cursor->col--;
      cursor->col -= utf_head_off(line, line + cursor->col);
      *pincl = true;
    }
  } else {
    cursor->col = 0;
...
/// cls() - returns the class of character at *cursor
///
...
int fwd_word(int count, bool bigword, bool eol)
{
  ...
  while (--count >= 0) {
    ...
    while (cls() == 0) {
      ...
      if (cursor->col == 0 && *get_cursor_line_ptr() == NUL) {
        break;
      }
...
int bck_word(int count, bool bigword, bool stop)
{
  ...
  while (--count >= 0) {
    ...
    // first line.
    if (hasFolding(curwin, cursor->lnum, &cursor->lnum, NULL)) {
      cursor->col = 0;
    }
    sclass = cls();
...
int current_word(oparg_T *oap, int count, bool include, bool bigword)
{
  ...
  if (!VIsual_active || equalpos(*cursor, *anchor)) {
  ...
  while (count > 0) {
    inclusive = true;
    if (VIsual_active && lt(*cursor, *anchor)) {
  ...
  if (include_white && (cls() != 0
    ...
    pos_T pos = *cursor;     // save cursor position
    *cursor = start_pos;
    if (oneleft() == OK) {
      back_in_line();
      if (cls() == 0 && cursor->col > 0) {
        if (VIsual_active) {
          *anchor = *cursor;
        } else {
          oap->start = *cursor;
    ...
    *cursor = pos;     // put cursor back at end
  }
...
int current_sent(oparg_T *oap, int count, bool include)
{
  ...
  if (VIsual_active && !equalpos(start_pos, *anchor)) {
extend:
    if (lt(start_pos, *anchor)) {
      ...
      incl(&pos);
      at_start_sent = true;
      if (!equalpos(pos, *cursor)) {     // not just before a sentence
        at_start_sent = false;
        while (lt(pos, *cursor)) {
          c = gchar_pos(&pos);
          if (!ascii_iswhite(c)) {
...
int current_block(oparg_T *oap, int count, bool include, int what, int other)
{
  ...
  pos_T old_end = *cursor;           // remember where we started
  pos_T old_start = old_end;

  // If we start on '(', '{', ')', '}', etc., use the whole block inclusive.
  if (!VIsual_active || equalpos(*anchor, *cursor)) {
    setpcmark();
    ...
    old_start = *anchor;
    *cursor = *anchor;              // cursor at low end of Visual
  } else {
    old_end = *anchor;
  }
  ...
  *cursor = *end_pos;
  ...
  while (!include) {
    ...
    if (!lt(start_pos, old_start) && !lt(old_end, *cursor)
        && !equalpos(start_pos, *cursor)
        && VIsual_active) {
      *cursor = old_start;
      decl(cursor);
      if ((pos = findmatch(NULL, what)) == NULL) {
  ...
  if (VIsual_active) {
    ...
    *anchor = start_pos;
    ...
    if (sol) {
      incl(cursor);
    } else if (ltoreq(start_pos, *cursor)) {
...
again:
  ...
  if (do_include) {
    ...
    if (*c == '<' && !VIsual_active && cursor->col == 0) {
...
int current_par(oparg_T *oap, int count, bool include, int type)
{
  ...
  linenr_T start_lnum = cursor->lnum;

  // When visual area is more than one line: extend it.
  if (VIsual_active && start_lnum != anchor->lnum) {
extend:
    dir = start_lnum < anchor->lnum ? BACKWARD : FORWARD;
    for (int i = count; --i >= 0;) {
      if (start_lnum ==
  ...
  if (VIsual_active) {
    ...
    if (VIsual_mode == 'V' && start_lnum == cursor->lnum) {
      goto extend;
    }
    if (anchor->lnum != start_lnum) {
      anchor->lnum = start_lnum;
      anchor->col = 0;
    }
    VIsual_mode = 'V';
    redraw_curbuf_later(UPD_INVERTED);  // update the inversion
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (VIsual_active) {
    ...
    if (*p_sel == 'e') {
      ...
      if (!vis_bef_curs && !vis_empty) {
        // *anchor needs to be start of Visual selection.
        pos_T t = *cursor;
  ...
  if (!vis_empty) {
    ...
    // quotes.
    if (vis_bef_curs) {
      inside_quotes = anchor->col > 0
                      && (uint8_t)line[anchor->col - 1] == quotechar
                      && line[cursor->col] != NUL
                      && (uint8_t)line[cursor->col + 1] == quotechar;
      i = anchor->col;
      col_end = cursor->col;
    } else {
      inside_quotes = cursor->col > 0
                      && (uint8_t)line[cursor->col - 1] == quotechar
                      && line[anchor->col] != NUL
                      && (uint8_t)line[anchor->col + 1] == quotechar;
      i = cursor->col;
